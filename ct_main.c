...
#include "regs.h" //contains the register enum for PIN
...
static VG_REGPARM(1) void trace_reg_read( PIN_REG r )
{
	if ( !tracing ) return;
    Int already_found = 0;
    for(Int i=0; i<NUM_INSTR_SOURCES; i++)
...
static VG_REGPARM(1) void trace_reg_write( PIN_REG r )
{
	if ( !tracing ) return;
    Int already_found = 0;
    for(Int i=0; i<NUM_INSTR_DESTINATIONS; i++)
...
static VG_REGPARM(0) void trace_branch_direct( void )
{
	if ( !tracing ) return;
...
static VG_REGPARM(0) void trace_branch_indirect( void )
{
	if ( !tracing ) return;
	...
	/* Hack : See note above */
	inst.destination_registers[0] = REG_RIP;
	inst.source_registers[0] = REG_RAX;
	if( DEBUG_CT ) {
...
static VG_REGPARM(0) void print_inst ( void )
{
   ...
   VG_(printf)(" %d :", inst.branch_taken	);
	...
	for(Int i = 0; i < NUM_INSTR_DESTINATIONS; i++)
	{
		VG_(printf)(" %08lx :", inst.destination_memory[i] );
	#ifdef TRACE_MEM_VALUES
		/*if( inst.destination_memory[i] != 0) {
			for(Int j = 0; j < CACHE_LINE_SIZE; j++)
			{
	...
	for(Int i = 0; i < NUM_INSTR_SOURCES ; i++)
	{
		VG_(printf)(" %08lx :", inst.source_memory[i] );
	#ifdef TRACE_MEM_VALUES
		/*if( inst.source_memory[i] != 0) {
			for(Int j = 0; j < CACHE_LINE_SIZE; j++)
			{
...
static void instrument_instruction ( IRSB* sb, IRAtom* iaddr, UInt isize )
{
   ...
   IRDirty*   di;
	...
	di = unsafeIRDirty_0_N( 0, "inc_inst", 
						VG_(fnptr_to_fnentry)( inc_inst ),
						mkIRExprVec_0() );
	...
	di = unsafeIRDirty_0_N( 0, "zero_inst", 
										VG_(fnptr_to_fnentry)( zero_inst ),
										mkIRExprVec_0() );
	addStmtToIRSB( sb, IRStmt_Dirty(di) );
...
static
IRExpr * cache_block_addr( const IRAtom * a )
{
   ...
   if (a->tag == Iex_RdTmp) {
      return mkIRExpr_HWord( ( a->Iex.RdTmp.tmp >> CACHE_POW ) << CACHE_POW );
		...
		Addr ad = (sizeof(Addr) == 4) ? a->Iex.Const.con->Ico.U32 : a->Iex.Const.con->Ico.U64;
...
static
void instrument_load ( IRSB* sb, IRAtom* daddr, Int dsize, IRAtom* guard )
{
   ...
   tl_assert(dsize >= 1 && dsize <= MAX_DSIZE);
	...
	di_mem->cee = mkIRCallee( 2/*reg params*/,"trace_load", VG_(fnptr_to_fnentry)(trace_load));
...
static
void instrument_store ( IRSB* sb, IRAtom* daddr, Int dsize, IRAtom* guard )
{
   tl_assert(isIRAtom(daddr));
   tl_assert(dsize >= 1 && dsize <= MAX_DSIZE);
	...
	IRExpr ** argv_mem = mkIRExprVec_2( daddr, mkIRExpr_HWord( dsize ) );
	di_mem->args=argv_mem;
	di_mem->cee = mkIRCallee( 2/*reg params*/,"trace_store", VG_(fnptr_to_fnentry)(trace_store));
	di_mem->guard = IRExpr_Const(IRConst_U1(True));
...
/* TODO This function is incomplete for now. Needs to be enhanced to support all registers */
static
PIN_REG offset_to_x86_64_register( Int offset , Int sz )
{
	tl_assert( offset >= 0 );
	...
	if(offset < 16) {
		...
		switch ( ( offset  - 216 ) / 32 ) {
			case 0: 
				return REG_YMM0; break;
			case 1: 
				return REG_YMM1; break;
			case 2: 
				return REG_YMM2; break;
			case 3: 
				return REG_YMM3; break;
			case 4: 
				return REG_YMM4; break;
			case 5: 
				return REG_YMM5; break;
			case 6: 
				return REG_YMM6; break;
			case 7: 
				return REG_YMM7; break;
			case 8: 
				return REG_YMM8; break;
			case 9: 
				return REG_YMM9; break;
			case 10: 
				return REG_YMM10; break;
			case 11: 
				return REG_YMM11; break;
			case 12: 
				return REG_YMM12; break;
			case 13: 
				return REG_YMM13; break;
			case 14: 
				return REG_YMM14; break;
			case 15: 
				return REG_YMM15; break;
			case 16:
...
static 
void instrument_reg_read( IRSB * sb, Int offset, Int sz )
{
	PIN_REG p;
	p = offset_to_x86_64_register( offset , sz );
	IRExpr ** argv = mkIRExprVec_1(mkIRExpr_HWord( p ));
	IRDirty * di = unsafeIRDirty_0_N( 1, "trace_reg_read",
...
static 
void instrument_reg_write( IRSB * sb, Int offset , Int sz )
{
	PIN_REG p;
	p = offset_to_x86_64_register( offset , sz );
	IRExpr ** argv = mkIRExprVec_1(mkIRExpr_HWord( p ));
	IRDirty * di = unsafeIRDirty_0_N( 1, "trace_reg_write",
							VG_(fnptr_to_fnentry)(
								trace_reg_write ),
							argv );
	addStmtToIRSB( sb, IRStmt_Dirty(di) );
...
static
void instrument_branch_conditional( IRSB * sb , Bool ci , IRExpr * guard )
{
	...
	IRDirty * di = unsafeIRDirty_0_N( 2, "trace_branch_conditional",
							VG_(fnptr_to_fnentry)(
								trace_branch_conditional ),
							argv );
	addStmtToIRSB( sb, IRStmt_Dirty(di) );
}

static
void instrument_branch_direct( IRSB * sb  )
{
	IRDirty * di = unsafeIRDirty_0_N( 0, "trace_branch_direct",
							VG_(fnptr_to_fnentry)(
								trace_branch_direct ),
							mkIRExprVec_0() );
	addStmtToIRSB( sb, IRStmt_Dirty(di) );
...
static
void instrument_branch_indirect( IRSB * sb  )
{
	IRDirty * di = unsafeIRDirty_0_N( 0, "trace_branch_indirect",
							VG_(fnptr_to_fnentry)(
								trace_branch_indirect ),
							mkIRExprVec_0() );
	addStmtToIRSB( sb, IRStmt_Dirty(di) );
}
/*------------------------------------------------------------*/
...
static void ct_post_clo_init(void)
{
	...
	VG_(printf)("inst struct size : %lu\n",sizeof(inst));
   ...
   for (/*use current i*/; i < sbIn->stmts_used; i++) {
      ...
      switch (st->tag) {
			...
			case Ist_IMark:
				...
				instrument_instruction(sbOut, mkIRExpr_HWord( (HWord)st->Ist.IMark.addr), ilen );
         ...
         case Ist_Put: {
				...
				instrument_reg_write( sbOut, st->Ist.Put.offset ,
						sizeofIRType( type) );
				addStmtToIRSB( sbOut, st );
				break;
			...
			case Ist_StoreG: {
				...
				IRType    type = typeOfIRExpr(tyenv, data);
				tl_assert(type != Ity_INVALID);
				instrument_store( sbOut, sg->addr,
	...
	case Ist_LoadG: {
			...
			case Ist_LLSC: {
				...
				if (st->Ist.LLSC.storedata == NULL) {
					...
					instrument_load( sbOut, st->Ist.LLSC.addr,
         ...
         case Ist_Exit:
            ...
            {
					instrument_branch_conditional( sbOut, condition_inverted , st->Ist.Exit.guard );
				}
...
static void ct_pre_clo_init(void)
{
   ...
   VG_(details_description)     ("generate Traces for Data ChampSim");
