...
#endif
		if( DEBUG_CT ) {VG_(printf)(" Direct Call\n"); }
	} 
	else if ( jk == Ijk_Ret ) {
#if defined(VGP_arm64_linux)
...
static VG_REGPARM(1) void trace_branch_indirect( IRJumpKind jk )
{
	...
	inst.is_branch = 1;
	inst.branch_taken = 1;
	if( DEBUG_CT ) {VG_(printf)(" Indirect Branch\n"); }
	/* Hack : See note above */
	if ( jk == Ijk_Call ) {
#if defined(VGP_arm64_linux)
		inst.destination_registers[0] = REG_PC;
		...
		inst.source_registers[0] = REG_PC;
		/* It could read from any register, we always mark it as REG_X30 for simplicity */
...
#endif
		if( DEBUG_CT ) {VG_(printf)(" Indirect Call\n"); }
	} 
	else if ( jk == Ijk_Ret ) {
#if defined(VGP_arm64_linux)
		inst.destination_registers[0] = REG_PC;
		...
		inst.source_registers[2] = REG_RAX;
#endif
		if( DEBUG_CT ) {VG_(printf)(" Indirect Return\n"); }
	}
	else if ( jk == Ijk_Boring ) {
#if defined(VGP_arm64_linux)
		inst.destination_registers[0] = REG_PC;
...
void instrument_load ( IRSB* sb, IRAtom* daddr, Int dsize, IRAtom* guard )
{
   ...
   tl_assert(dsize >= 1 && dsize <= MAX_DSIZE);
	...
	di_mem->cee = mkIRCallee( 2,"trace_load", VG_(fnptr_to_fnentry)(trace_load));
...
void instrument_store ( IRSB* sb, IRAtom* daddr, Int dsize, IRAtom* guard )
{
   ...
   tl_assert(dsize >= 1 && dsize <= MAX_DSIZE);
	...
	IRExpr ** argv_mem = mkIRExprVec_2( daddr, mkIRExpr_HWord( dsize ) );
	di_mem->args=argv_mem;
	di_mem->cee = mkIRCallee( 2,"trace_store", VG_(fnptr_to_fnentry)(trace_store));
	di_mem->guard = IRExpr_Const(IRConst_U1(True));
...
#if defined(VGP_arm64_linux)
static
ARM64_REG offset_to_arm64_register( Int offset ) {
...
static
void instrument_branch_direct( IRSB * sb, IRJumpKind jk )
{
	
	IRExpr ** argv = mkIRExprVec_1(mkIRExpr_HWord( jk ) );
	IRDirty * di = unsafeIRDirty_0_N( 1, "trace_branch_direct",
							VG_(fnptr_to_fnentry)(
								trace_branch_direct ),
							argv );
	addStmtToIRSB( sb, IRStmt_Dirty(di) );
...
static
void instrument_branch_indirect( IRSB * sb, IRJumpKind jk  )
{
	IRExpr ** argv = mkIRExprVec_1(mkIRExpr_HWord( jk ) );
	IRDirty * di = unsafeIRDirty_0_N( 1, "trace_branch_indirect",
							VG_(fnptr_to_fnentry)(
